...
/**
 * Servlet implementation class Producerregistercontoller
 */
@WebServlet("/Producerregistercontoller")
public class Producerregistercontoller extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Producerregistercontoller() {
        super();
 ...
 if(occupation==null|occupation.length()==0){
   ...
   if(error.isEmpty()){
       ...
       Producersregistermodel prm=new Producersregistermodel();
        ...
        prm.setLocality(locality);
        prm.setName(name);
        prm.setOccupation(occupation);
        prm.setPassword(password);
        prm.setPno(pno);
        prm.setProducts(products);
        prm.setUname(uname);
        ...
        try {
    		...
    		PreparedStatement prepareStatement = connection.prepareStatement("insert into producer values(null,?,?,?,?,?,?,?,?,'images/9.jpg')");
    		prepareStatement.setString(1, prm.getName());
    		prepareStatement.setString(2, prm.getUname());
    		prepareStatement.setLong(3,pnoi);
    		prepareStatement.setString(4,prm.getLocality());
    		prepareStatement.setString(5,prm.getAddress());
    		prepareStatement.setString(6,prm.getOccupation());
    		prepareStatement.setString(7,prm.getProducts());
    		prepareStatement.setString(8,prm.getPassword());
    	   ...
    	   int result =prepareStatement.executeUpdate();
       		...
       		if(result>0){
       			...
       			request.setAttribute("register", prm);
