...
/**
 * Servlet implementation class Producerproductcontroller
 */
@WebServlet("/Producerproductcontroller")
public class Producerproductcontroller extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Producerproductcontroller() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   if(error.isEmpty()){
	       ...
	       p.setUname(prm.getUname());
	       p.setUtype("Producer");
	       p.setPname(pname);
	       p.setQuantity(quantity);
	       p.setPrice(price);
	       p.setDescription(description);
	       p.setLocality(prm.getLocality());
	       p.setAddress(prm.getAddress());
	       
	        try {
	    		Context context = new InitialContext();
	    		DataSource ds=(DataSource)context.lookup("java:comp/env/jdbc/test");
	    		Connection connection = ds.getConnection();
	    		PreparedStatement prepareStatement = connection.prepareStatement("insert into product values(null,?,?,?,?,?,?,?,?,'images/311.jpg')");
	    	   ...
	    	   int result =prepareStatement.executeUpdate();
	       		...
	       		if(result>0){
	       			...
	       			request.getRequestDispatcher("produceraddproduct.jsp").forward(request, response);
